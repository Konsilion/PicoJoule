...
// Lancement manuel d'un chargement de Katalog

function KatalogGenerator(path) {
    
    var fullpath = path.resolve(path + "../data/data.csv");
    
    console.log(fullpath);
    
    // -----> Données de votre liste de projet - Gridcard
    Papa.parse(path + "/data/data.csv", { 
        download: true,
        delimiter: ";",
        skipEmptyLines: true,
        complete: results => {
    ...
    // -----> Vos paramètres
    Papa.parse(path + "/data/parametres.csv", { 
        download: true,
        delimiter: ";",
        skipEmptyLines: true,
        complete: results => {
